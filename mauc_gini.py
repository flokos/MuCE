...
def MultiClassNormalizedGini(labels,probas,num_classes,dt_labels):
    k = list(labels)
    for n,i in enumerate(labels):
        k[n] = np.int64(list(np.unique(dt_labels)).index(i))
    
    data = zip(k,probas)
    return (2*MAUC(data,num_classes=num_classes))-1
